...
public class StoreDetailServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        JsonNode storeJson = mapper.valueToTree(new StoreMessage(MessageStatus.OK, "Success", storeEntity));

        try {
            addUserEstimateInQueueToJson(storeJson);
        } catch (BadTicketException | BadStoreException e) {
            out.print(ow.writeValueAsString(new Message(MessageStatus.ERROR, e.getMessage())));
            return;
        }

        out.print(storeJson.toPrettyString());
    }

    private void addUserEstimateInQueueToJson(JsonNode rootNode) throws BadTicketException, BadStoreException {
        JsonNode s = rootNode.get("store");

        int custNum = ticketService.getCustomersQueue(s.get("storeId").asInt());
        int estimateTime = storeService.getEstimateTime(s.get("storeId").asInt());
        ((ObjectNode) s).put("customersInQueue", custNum);
        ((ObjectNode) s).put("estimateTime", estimateTime);
    }
}